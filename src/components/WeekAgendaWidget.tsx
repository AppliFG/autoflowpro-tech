...
const EVENT_TYPES: Record<string, { label: string; dotClass: string }> = {
  garage: { label: "Garage", dotClass: "bg-red-500" },
  carrosserie: { label: "Carrosserie", dotClass: "bg-orange-500" },
  preparateur: { label: "Préparateur", dotClass: "bg-violet-500" },
  attente_reception: { label: "Réception", dotClass: "bg-amber-500" },
  attente_livraison: { label: "Livraison", dotClass: "bg-sky-500" },
  vendu_livraison: { label: "Livraison vendu", dotClass: "bg-emerald-500" },
};
...
export default function WeekAgendaWidget() {
  ...
  const eventsByDay = days.map((day) => {
    ...
    return {
      date: day,
      dateStr,
      events: events.filter((e: any) => e.event_date === dateStr),
    };
  });
              ...
              <div className="space-y-1">
                {dayEvents.slice(0, 3).map((ev: any) => {
                  ...
                  return (
                    <div
                      key={ev.id}
                      className="flex items-start gap-1 rounded-md bg-background/80 p-1.5 text-[10px] leading-tight"
                    >
