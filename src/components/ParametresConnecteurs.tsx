...
import { Plug, ExternalLink, Search, Eye, EyeOff, Save, Unplug, Info } from "lucide-react";
...
export default function ParametresConnecteurs() {
  ...
  const loadCredentials = async () => {
    ...
    for (const row of data) {
      const match = row.key.match(/^connector_(.+?)_(token|login|password|enabled)$/);
      if (!match) continue;
      const [, cid, field] = match;
      if (!creds[cid]) creds[cid] = { connecteur_id: cid, enabled: false };
      if (field === "enabled") creds[cid].enabled = row.value === "true";
      else (creds[cid] as any)[field] = row.value;
    }
  ...
  const saveConnector = async (c: ConnecteurConfig) => {
    ...
    try {
      ...
      const settings: { key: string; value: string }[] = [
        { key: `connector_${c.id}_enabled`, value: String(cred.enabled) },
      ];
      if (c.authType === "token") {
        settings.push({ key: `connector_${c.id}_token`, value: cred.token || "" });
      } else {
        settings.push({ key: `connector_${c.id}_login`, value: cred.login || "" });
        settings.push({ key: `connector_${c.id}_password`, value: cred.password || "" });
      }
      for (const s of settings) {
        const { error } = await supabase.from("app_settings").upsert(s, { onConflict: "key" });
        if (error) throw error;
      }
      toast.success(`${c.name} enregistré`);
    } catch (err: any) {
  ...
  const disconnect = async (c: ConnecteurConfig) => {
    try {
      const keys = [`connector_${c.id}_enabled`, `connector_${c.id}_token`, `connector_${c.id}_login`, `connector_${c.id}_password`];
  ...
  return (
    <div className="space-y-4">
      ...
      {Object.entries(grouped).map(([cat, connectors]) => (
        <div key={cat}>
          <h4 className="text-sm font-semibold text-muted-foreground mb-2">{CATEGORY_LABELS[cat]}</h4>
      ...
      <div className="rounded-lg bg-muted/50 p-4 flex items-start gap-3">
        ...
        <div className="text-xs text-muted-foreground">
          <p className="font-semibold text-card-foreground mb-1">Architecture SaaS multi-tenant</p>
          <p>Chaque compte dispose de ses propres identifiants fournisseurs. Les credentials sont isolés entre les comptes et ne sont jamais partagés. Seul l'administrateur de votre espace peut configurer ces accès.</p>
        </div>
      </div>
    </div>
  );
